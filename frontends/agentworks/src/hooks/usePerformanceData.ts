...
import { searchTalents, type SearchTalentsParams, type DashboardStats } from '../api/talent';
...
export function usePerformanceData(platform: Platform, filterParams?: PerformanceFilterParams) {
  ...
  // 保存当前筛选参数的引用，用于比较
  const filterParamsRef = useRef<PerformanceFilterParams | undefined>(filterParams);

  const loadData = useCallback(async (page: number = currentPage, filters: PerformanceFilterParams = filterParams || {}) => {
    try {
      setLoading(true);
      ...
      if (filters.tiers) {
        ...
        } else if (typeof filters.tiers === 'string' && filters.tiers.trim()) {
          // 字符串格式转数组
          params.tiers = filters.tiers.split(',').map((s: string) => s.trim()).filter(Boolean);
        }
      }
      ...
      if (filters.maleRatioMin !== undefined) params.maleRatioMin = filters.maleRatioMin;
      if (filters.maleRatioMax !== undefined) params.maleRatioMax = filters.maleRatioMax;
      if (filters.femaleRatioMin !== undefined) params.femaleRatioMin = filters.femaleRatioMin;
      if (filters.femaleRatioMax !== undefined) params.femaleRatioMax = filters.femaleRatioMax;
  ...
  // 执行搜索（带筛选参数）
  const search = useCallback((filters: PerformanceFilterParams) => {
