...
#### 达人列表返点显示
- 实现从 talents 集合读取 currentRebate 字段
- 支持 currentRebate 对象结构（rate/source/effectiveDate/lastUpdated）
- 返点率统一保留 2 位小数显示（如 40.00%）

#### 类型定义更新
- Talent 接口添加 currentRebate 对象类型
- TalentListItem 接口同步更新
- 支持返点来源追踪（default/personal/rule/agency）

#### 云函数优化 (getTalents v3.1)
- v2 模式自动返回 currentRebate 对象
- 移除不必要的数据映射逻辑
- 保持向后兼容性

#### 格式化工具增强
- formatRebate 函数支持字符串和数字输入
- 自动处理无效值返回 "-"
- 统一保留 2 位小数格式

### 🐛 Bug 修复
- 修复 formatRebate 函数 toFixed 错误
- 修复 currentRebate 字段类型不匹配问题
- 处理字段可能为字符串的兼容性问题
