...
- 一致的组件样式和交互模式
- 统一的颜色系统（靛蓝主色调 `primary-600: #4f46e5`）
- Ant Design 与 Tailwind 颜色配置保持一致
...
**主色调（靛蓝色系）** - 统一使用 `primary-*` 类名：
```tsx
// 主色调
className="bg-primary-600 text-white hover:bg-primary-700"
className="bg-primary-50 text-primary-700"  // 浅色背景

// Tailwind 配置中的 primary 色值
primary-50:  '#eef2ff'
primary-100: '#e0e7ff'
primary-500: '#6366f1'
...
- ✅ **Ant Design 主题配置**：添加 ConfigProvider，颜色与 Tailwind 保持一致
- ✅ **全局样式重构**：`index.css` 重写为设计系统，添加 CSS 变量和组件类
- ✅ **侧边栏优化**：改为浅色主题，宽度 192px，添加展开/收起动画
- ✅ **主内容区布局**：最大宽度调整为 1500px
- ✅ **字体系统**：统一使用 Inter + 苹方
- ⚠️ **禁止使用 `blue-*`**：统一使用 `primary-*` 类名
